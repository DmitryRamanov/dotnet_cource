...
void ReversFirstAndLastRowsInArray2D(int[,] array)
{
    int rowsNumber = array.GetLength(0);
    int columnsNumber = array.GetLength(1);
    ...
    for (int columnIndex = 0; columnIndex < columnsNumber; columnIndex++)
    {
        // int tempValue = array[0, columnIndex];
        // array[0, columnIndex] = array[rowsNumber - 1, columnIndex];
        // array[rowsNumber - 1, columnIndex] = tempValue;
        //(A, B) = (B, A + B);
        (array[0, columnIndex], array[rowsNumber - 1, columnIndex])
            = (array[rowsNumber - 1, columnIndex], array[0, columnIndex]);
    }
}
...
ReversFirstAndLastRowsInArray2D(array2D);
Console.WriteLine();
PrintArray2D(array2D);